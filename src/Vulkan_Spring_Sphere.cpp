...
static std::vector<uint32_t> generateSphereIndices(uint32_t width, uint32_t height, uint32_t depth) {
    ...
    for (uint32_t y = 0; y < height - 1; y++) {
        for (uint32_t x = 0; x < width - 1; x++) {
            ...
            // 뒷면
            addQuad(getIdx(x + 1, y, depth - 1), getIdx(x + 1, y + 1, depth - 1),
                getIdx(x, y + 1, depth - 1), getIdx(x, y, depth - 1));
        }
    }
    ...
    for (uint32_t z = 0; z < depth - 1; z++) {
        for (uint32_t y = 0; y < height - 1; y++) {
            ...
            // 오른쪽면
            addQuad(getIdx(width - 1, y + 1, z), getIdx(width - 1, y + 1, z + 1), 
                getIdx(width - 1, y, z + 1), getIdx(width - 1, y, z));
        }
    }
    ...
    for (uint32_t z = 0; z < depth - 1; z++) {
        for (uint32_t x = 0; x < width - 1; x++) {
            ...
            // 윗면
            addQuad(getIdx(x, height - 1, z), getIdx(x, height - 1, z + 1), 
                getIdx(x + 1, height - 1, z + 1), getIdx(x + 1, height - 1, z));
        }
    }
...
private:
    ...
    void mainLoop() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // 1.0초(1000ms)가 경과할 때마다 FPS 갱신
            if (currentTime - lastTime >= 0.016) {
                // 창 제목에 텍스트와 FPS 결합
                std::string title = "Vulkan Cloth Simulation - FPS: " + std::to_string(nbFrames / 0.016);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        int computeSetIndex = (currentFrame + (pc.subStepCnt % 2 == 0 ? 1 : 0)) % MAX_FRAMES_IN_FLIGHT;

        for (int count = 0; count < pc.subStepCnt; count++) {
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout,
                0, 1, &computeDescriptorSets[computeSetIndex], 0, nullptr);
            vkCmdDispatch(commandBuffer, 1, 1, 16);
            addComputeBarrier(commandBuffer, shaderStorageBuffers[computeSetIndex]);
            computeSetIndex = (computeSetIndex + 1) % MAX_FRAMES_IN_FLIGHT;
        }
